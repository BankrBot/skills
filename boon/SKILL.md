...
3. **Normalize identities before execution.** Valid handles are `github:<user>`,
   `x:<user>`, or `agent:<positive decimal id>`. Reject `agent:0`, leading
   zeroes, hex ids, whitespace, and ambiguous usernames.
...
history, and claim device-code flows. Browser-wallet users can use the web app
at https://boonprotocol.com/send.
