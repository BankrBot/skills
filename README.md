...
| [Aeon](https://github.com/aaronjmars/aeon) | [aeon-autoresearch](aeon-autoresearch/) | Evolve any skill by generating four distinct improved variations along separate theses, scoring them on a weighted rubric (improvement, output value, clarity, data quality, robustness, conventions), and shipping the winning version as a PR. Never downgrades a working skill. |
| [Aeon](https://github.com/aaronjmars/aeon) | [aeon-spawn-instance](aeon-spawn-instance/) | Clone a parent agent into a new GitHub repo with a tailored skill set — fork, configure, validate, enable Actions, register in the fleet, hand back to the operator pending secrets. Self-replicating agent infrastructure with no secret propagation. |
| [Aeon](https://github.com/aaronjmars/aeon) | [aeon-skill-repair](aeon-skill-repair/) | Auto-diagnose and fix failing or degraded skills. Systemic-first triage (one shared fix for N skills hitting the same root cause), per-category playbooks (api-change, rate-limit, timeout, sandbox-limitation, prompt-bug, output-format, missing-secret, config), verification block attached to every PR. |
| [Aeon](https://github.com/aaronjmars/aeon) | [aeon-skill-security-scan](aeon-skill-security-scan/) | Audit skills, workflows, and companion scripts for injection, exfiltration, traversal, prompt-override, destructive commands, and obfuscation risks. Delta tracking against prior runs, baseline suppression, issue filing for NEW HIGH findings, per-finding remediation. Designed to integrate with Bankr Safety Scores. |
...
| [Aeon](https://github.com/aaronjmars/aeon) | [aeon-workflow-security-audit](aeon-workflow-security-audit/) | Audit GitHub Actions workflows with zizmor + actionlint plus hand-rolled checks (toJson-into-shell, persist-credentials + ref poisoning, GITHUB_ENV injection). Delta classification (NEW/REINTRODUCED/UNCHANGED/RESOLVED), auto-fix Critical/High regressions via env: rebind, PR only on non-empty delta. |
...
| [Aeon](https://github.com/aaronjmars/aeon) | [aeon-skill-evals](aeon-skill-evals/) | Validate skill outputs against assertion manifests (word counts, required/forbidden patterns, output shape, source citation). Diff vs prior runs to flag NEW_FAIL / NEW_PASS / CHRONIC regressions, file issues, queue concrete fixes. Bootstrap mode generates starter manifests from real historical output. |
| [Aeon](https://github.com/aaronjmars/aeon) | [aeon-fleet-control](aeon-fleet-control/) | Parent-side control plane for multi-agent topologies — health checks, cross-fleet dispatch, output aggregation, quarantine. When instances use Bankr-backed wallets, surfaces cross-fleet treasury totals via per-instance Wallet API lookups. |
| [Aeon](https://github.com/aaronjmars/aeon) | [aeon-operator-scorecard](aeon-operator-scorecard/) | Weekly plain-language synthesis of agent health + community growth + economic activity. Answers "was this week worth it?" with a single verdict (WORTH IT / MIXED / STRUGGLING / CRISIS) reproducible from named inputs. Pulls Bankr Wallet balances for the economic layer. |
| [Aeon](https://github.com/aaronjmars/aeon) | [aeon-schedule-ads](aeon-schedule-ads/) | Schedule paid ads across Meta / TikTok / Snapchat / Pinterest / LinkedIn via the AdManage.ai API from a declarative config. Launches PAUSED by default — never auto-activates live spend. Idempotent provisioning, separated activate operation for blast-radius isolation. |
