#!/usr/bin/env node
// hood-stock-lp ledger: what the user's Uniswap positions on Robinhood Chain
// actually earned, split the way a market maker's books split it: fees vs
// value taken by informed flow vs impermanent loss vs gas, per $1k. Answers
// "I made $80 in fees, but how much did informed flow take back?"
//
...
//
//   --hedge hl   COMING, not in this version: the Hyperliquid hedge leg
//                (hedge P&L, funding). Refused with a clear message today.
//
// Data: DeltaDesk's tearsheet (https://web-production-10951.up.railway.app).
// It covers the NVDA, SPY and TSLA markets of this skill; other markets are
// reported as not covered, never silently dropped.
...
function main() {
  if (args.hedge !== undefined) {
    out(
      {
        ok: false,
        gate: "hedge-coming",
        detail:
          "--hedge hl is coming but not in this version: it will add the Hyperliquid hedge leg (hedge P&L and funding) next to the LP lines. Today the ledger is the unhedged LP book.",
        next: "Run the same command without --hedge.",
      },
      2
    );
  }
