...
Running inside Meta Muse or another cloud-VM agent with a secure credential store? Follow [references/muse.md](references/muse.md) so the key lands in the store, not on disk — reuse the key login just created (never mint a second one), verify with `bankr whoami`, then `bankr logout`.
