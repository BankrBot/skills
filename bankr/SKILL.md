...
Running inside Meta Muse or another cloud-VM agent with a secure credential store? Follow [references/muse.md](references/muse.md) so the key lands in the store, not on disk — the user rotates their existing key at bankr.bot/api-keys and adds the new one to the store (never mint a second key, never read it from `~/.bankr/config.json`), then verify with `bankr whoami`.
