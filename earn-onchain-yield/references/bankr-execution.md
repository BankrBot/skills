...
4. Show the full plan, including **every** approval transaction, spender, cap, Permit2 expiry, estimated gas, intended principal and supported minimum output. Distinguish estimates from contract-enforced guarantees: Auto direct deposits have minimum underlying amounts used, but no minimum receipt shares or onchain expiry. Disclose those limitations before approvals and stop if the user requires either missing protection, as specified in the protocol reference. Ask the user to confirm this bounded plan. A yield question or skill installation alone is not permission to spend. A quote refresh may proceed within the confirmed bounds; changes to the product, amount, spender or a less protective minimum require renewed confirmation.
...
6. Refresh quotes after approvals. Apply the protocol reference's `validateQuote` guard to every swap leg: exact requested input, minimum output consistent with approved slippage, and a valid millisecond timestamp aged **0–30 seconds**, never in the future. For calls with a deadline argument, require it to be strictly after the current chain timestamp and no more than **10 minutes** ahead; never invent a deadline for a call without one. Recheck wallet balances and simulate the **complete final transaction** with `eth_call` from the Bankr wallet with actual allowances in place. A quote marked `simulationIncomplete` cannot replace this check. Estimate gas for that same calldata. Never ignore a failed final simulation. Immediately before submission, after those operations, recheck every quote and any deadline. If stale, expired or invalid, rebuild and simulate fresh quotes within confirmed bounds instead of sending the old payload.
